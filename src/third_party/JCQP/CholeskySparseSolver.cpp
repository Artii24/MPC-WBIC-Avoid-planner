...
#ifdef JCQP_USE_AVX2
__attribute__((unused)) static void spTriAVX(u32 n, const u32* p, const u32* i, const double* v, double* x)
{
...
__attribute__((unused)) static void spTriTAVX(u32 n, const u32* p, const u32* i, const double* v, double* x)
{
 ...
 */
__attribute__((unused)) static void spTriTAVX2(u32 n, const u32* p, const u32* i, const double* v, double* x)
{
  constexpr int UNROLL = 1;
  for (s32 c = n - 1; c >= 0; c--)
  {                                 // loop backward over columns
...
void CholeskySparseSolver<T>::amdOrder(MatCSC<T>& mat, u32* perm, u32* iperm)
{
  ...
  if (amdRV < 0)
    throw std::runtime_error("reorder failed");
